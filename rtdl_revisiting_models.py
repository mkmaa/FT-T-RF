...
class LinearEmbeddings(nn.Module):
    ...
    **Examples**
    ...
    def reset_parameters(self) -> None:
        ...
        d_qsrt = self.weight.shape[1] ** -1
        self.relu = nn.ReLU() # for random feature
        
        if args.init == 'xavier_uniform':
            print('linear embedding: xavier_uniform_ -- d_rqsrt =', d_rqsrt) # define init, default xavier uniform
            ...
            print('linear embedding: xavier_normal_ -- d_qsrt =', d_qsrt)
            nn.init.normal_(self.weight, 0, d_qsrt)
            if args.bias == 'True':
                nn.init.normal_(self.bias, 0, d_qsrt)
            else:
                nn.init.zeros_(self.bias)
        elif args.init == 'kaiming_uniform':
            print('linear embedding: kaiming_uniform_')
            nn.init.kaiming_uniform_(self.weight, 0, args.fan, 'relu')
            if args.bias == 'True':
                nn.init.kaiming_uniform_(self.bias, 0, args.fan, 'relu')
            else:
                nn.init.zeros_(self.bias)
        elif args.init == 'kaiming_normal':
            print('linear embedding: kaiming_normal_')
            nn.init.kaiming_normal_(self.weight, 0, args.fan, 'relu')
            if args.bias == 'True':
                nn.init.kaiming_normal_(self.bias, 0, args.fan, 'relu')
            else:
                nn.init.zeros_(self.bias)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # print('before:', x.shape, self.weight.shape, self.bias.shape)
...
class CategoricalEmbeddings(nn.Module):
    ...
    **Examples**
    ...
    def reset_parameters(self) -> None:
        print('categorical embedding: uniform_')
    ...
    **Examples**
    ...
    def __init__(
        ...
        *,
        ...
        ffn_activation: _TransformerFFNActivation = 'ReGLU',  # define activation, default ReGLU
    ...
    def __init__(
        ...
        **backbone_kwargs,
        ...
        self.cont_embeddings = (
            LinearEmbeddings(n_cont_features, d_block)
            if n_cont_features > 0
            else None
        )
        self.cat_embeddings = (
